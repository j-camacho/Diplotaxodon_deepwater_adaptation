...
* **00_VCF/**
  Directory containing single nucleotide polymorphism (SNP) data in Variant Call Format (VCF) for the 79 sequenced Diplotaxodon included in this study (Supplementary Data 3). Files are organized per chromosome. See Methods for additional information.
...
* **06_fAstCal1.2/**
  Directory containing the reference FASTA and genomic annotation (GTF) for *Astatotilapia calliptera* (NCBI RefSeq assembly GCF_900246225.1), used for RNAseq read alignment.
  Retrieved on 27-10-2023 using:
  `curl -OJX GET "https://api.ncbi.nlm.nih.gov/datasets/v2alpha/genome/accession/GCF_900246225.1/download?include_annotation_type=GENOME_FASTA,GENOME_GTF,SEQUENCE_REPORT&filename=GCF_900246225.1.zip" -H "Accept: application/zip"`
...
* **06_fAstCal1.2-opsins.txt**
  ...
  * Start (bp)
  * End (bp)
* Raw RNAseq reads can be retrived from XXX.
